...
const char file_name_of_problem[] = "bouncyball";
...
lo random(lo miv, lo mav) {
    assert(miv <= mav);
    return miv + rand_t() * 1LL % (mav-miv+1);
}
...
void generate() {
    ...
    lo n = random(1, 10), m = random(1, 10);
    cout << "n: " << n << nln;
    cout << "m: " << m << nln;
    lo x1 = random(1, n);
    lo y1 = random(1, m);
    lo x2 = random(1, n);
    lo y2 = random(1, m);
    string drt[] = {"DL", "UL", "DR", "UR"};
    fi << n << ' ' << m << ' ' << x1 << ' ' << y1 << ' ' << x2 << ' ' << y2 << ' ' << drt[rand()%3] << nln;
    fi.close();
}
